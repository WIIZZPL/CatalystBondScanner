...
def data_splitter(input_data :pd.DataFrame, history_size:int, future_size:int, train_size:float) -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray):
    ...
    for i in range(history_size, len(train_data)-future_size):
        train_data_x.append(train_data[i - history_size:i])
        train_data_y.append(train_data[i:i + future_size])

    for i in range(history_size, len(test_data)-future_size):
        test_data_x.append(test_data[i - history_size:i])
        test_data_y.append(test_data[i:i + future_size])
